...
class ScanVbs(FormatString,OpenEncodingFile):
    """ This class is to implment functions/methods to scan uft lib files,
and organize them like class:function/sub/property and only deal with simple and normal comment."""
    def __init__(self,sp={}):
        print("nomal scan starts")
        if type(sp)!=type({}):
            raise TypeError("class ScanVbs needs a dictionary as parameter but now it is %s" %type(sp))
        FormatString.__init__(self,vbsdocStyle=False)
        self.fileType = filetype
        self.storePoint = sp
        OpenEncodingFile.__init__(self)
        return
    def __GetSectionName(self,line, section):
        """This function is to retrieve the name of the sector from the line depanding on the input sector name.
it first finds if the sectorname is contained, then try to get all string which is not part of comments and return them."""
        aList = line.split()
        count = len(aList)
                        ...
                        if not self.__CheckItemRecorded(sp,item ,name):
                            if item == self.sectorClass:
                                ...
                                if i>=0:
                                    ...
                                    mystr = line
                                arr.append(mystr.lstrip() + self.SoftReturn)
